...
if( ROBBIE_LIBRARIES AND ROBBIE_INCLUDE_DIRS )
    ...
    if( NOT ROBBIE_FIND_VERSION )
        if( NOT ROBBIE_FIND_VERSION_MAJOR )
            set( ROBBIE_FIND_VERSION_MAJOR 0 )
        endif( NOT ROBBIE_FIND_VERSION_MAJOR )
        if( NOT ROBBIE_FIND_VERSION_MINOR )
            set( ROBBIE_FIND_VERSION_MINOR 2 )
        endif( NOT ROBBIE_FIND_VERSION_MINOR )
        if( NOT ROBBIE_FIND_VERSION_PATCH )
            set( ROBBIE_FIND_VERSION_PATCH 0 )
        endif( NOT ROBBIE_FIND_VERSION_PATCH )
        set( ROBBIE_FIND_VERSION "${ROBBIE_FIND_VERSION_MAJOR}.${ROBBIE_FIND_VERSION_MINOR}.${ROBBIE_FIND_VERSION_PATCH}" )
    endif( NOT ROBBIE_FIND_VERSION )

    macro( check_version )
        file( READ "${ROBBIE_INCLUDE_DIR}/BuildVersion.h" BUILD_VERSION_HEADER )

        # Major version
        string( REGEX MATCH "define[ \t]+LIBRARY_MAJOR_VERSION[ \t]+[\"]([0-9]+)+[\"]" ROBBIE_MAJOR_VERSION_MATCH "${BUILD_VERSION_HEADER}" )
        set( ROBBIE_MAJOR_VERSION "${CMAKE_MATCH_1}" )

        # Minor version
        string( REGEX MATCH "define[ \t]+LIBRARY_MINOR_VERSION[ \t]+[\"]([0-9]+)+[\"]" ROBBIE_MINOR_VERSION_MATCH "${BUILD_VERSION_HEADER}" )
        set( ROBBIE_MINOR_VERSION "${CMAKE_MATCH_1}" )

        # Patch version
        string( REGEX MATCH "define[ \t]+LIBRARY_PATCH_VERSION[ \t]+[\"]([0-9]+)+[\"]" ROBBIE_PATCH_VERSION_MATCH "${BUILD_VERSION_HEADER}" )
